...
describe('Test video stored files', function () {
  ...
  it('Should rebuild HLS by copying the kept original file', async function () {
    ...
    // The removed 720p resolution must not be listed anymore
    expect(masterPlaylist.match(/#EXT-X-STREAM-INF/g)).to.have.lengthOf(1)
    expect(masterPlaylist).to.not.contain('-720.m3u8')

    for (const line of masterPlaylist.split('\n').filter(l => l.endsWith('.m3u8'))) {
      const resolutionPlaylistUrl = video.streamingPlaylists[0].playlistUrl.replace(/[^/]+$/, line)
      await server.streamingPlaylists.get({ url: resolutionPlaylistUrl })
    }
  })
